...
ds.ui.__experemental___DataConnection = ds.Object.extend({
	_get_cache: null,
	_get_cache_lifetime: 3000, // ms
	_queue: null,
	headers: null,
	host: null,
	async _processRequest(req) {
		const self = this;
		if (req.method.toLowerCase() == 'get' && req.options.cache) {
			let descr = req.method.toUpperCase() + '_' + decodeURIComponent(req.url);
			let cache = self._get_cache[descr];
			if (cache && (((new Date()).getTime() - cache.time) <= self._get_cache_lifetime)) {
				req.options.log && console.log(req.method.toUpperCase() + ' ' + decodeURIComponent(req.url) + ' (from cache)');
				req.resolve(ds.clone(cache.data));
				return;
			}
		}
		req.options.log && ds.ui.Progress.shared().show();
		let console_time_descr = req.method.toUpperCase() + ' ' + decodeURIComponent(req.url) + ' ' + ds.nextId();
		req.options.log && console.time(console_time_descr);
		let body_str = null;
		if (req.body) {
			if (req.body instanceof Object) body_str = JSON.stringify(req.body);
			else body_str = req.body.toString();
		}
		let json;
		try {
			let response = await fetch(self.host + req.url, { mode: 'cors', credentials: 'include', method: req.method.toUpperCase(), headers: new Headers(self.headers || {}), body: body_str  });
			if (response.status != 200) {
				req.reject(new Error('ds.ui.DataConnection.request: Request returned error code "' + response.status.toString() + '".'));
				return;
			}
			json = await response.json();
			if (json.status == 'error') {
				req.reject(new Error('ds.ui.DataConnection.request: Data source returned error "' + json.message + '".'));
				return;
			}
		} catch(e) {
			ds.ui.Progress.shared().hide();
			console.timeEnd(console_time_descr);
			req.reject(e);
			self._trigger('error', e);
		}
		req.options.log && console.timeEnd(console_time_descr);
		req.options.log && ds.ui.Progress.shared().hide();
		if (req.method.toLowerCase() == 'get' && req.options.cache) {
			let descr = req.method.toUpperCase() + '_' + decodeURIComponent(req.url);
			self._get_cache[descr] = { data: json.data, time: (new Date()).getTime() };
		}
		self._trigger('request', json);
		req.done = true;
		req.data = ds.clone(json.data);
	},
	async _processQueue() {
		const self = this;
		...
		let done = [];
		let req;
		while (req = self._queue.shift()) {
			await self._processRequest(req)
			done.push(req);
		}
		self._active = false;
		done.forEach(req => req.resolve(req.data));
	},
	async request(method, url, body, options) {
		const self = this;
		if (!self.host) throw new Error('ds.ui.DataConnection: Unable to perform request, "host" property is null.');
		return new Promise((resolve, reject) => {
			self._queue.push({
				...
				url: url,
				body: body,
				options: Object.assign({ log: true, cache: true }, options),
				done: false,
				data: null,
			});
			self._processQueue();
...
ds.ui.DataController = ds.Object.extend({
	host: null,
	url: null,
	method: null,
	headers: null,
	...
	async _fetch(url) {
		const self = this;
		if (!self.host) throw new Error('ds.ui.DataController: Host is required.');
		if (!self.method) throw new Error('ds.ui.DataController: Method is required.');
		let json;
		try {
			let res = await fetch(self.host + url, { mode: 'cors', credentials: 'include', method: self.method.toUpperCase(), headers: new Headers(self.headers || {}) });
			if (res.status != 200) throw new Error('ds.ui.DataController: Request returned error code "' + response.status.toString() + '".');	
			json = await res.json();
			if (!json.hasOwnProperty('status')) throw new Error('ds.ui.DataController: Result JSON has no property "status".');
			if (!(['error', 'success']).includes(json.status)) throw new Error('ds.ui.DataController: Only "error" and "success" values are allowed in result JSON property "status".');
			if (json.status == 'error') throw new Error('ds.ui.DataController: Data source returned error "' + json.message + '".');		
		} catch(e) {
			self.active = false;
			self._trigger('error', e);
			throw e;
		}
		if (!json.hasOwnProperty('dataType')) throw new Error('ds.ui.DataController: Result JSON has no property "dataType".');
		if (!json.dataType != 'dataset') throw new Error('ds.ui.DataController: Result JSON with dataType "' + json.dataType + '" not supported.');
		if (!json.hasOwnProperty('pageList')) throw new Error('ds.ui.DataController: Result JSON has no property "pageList" for "dataset" dataType.');
		if (!json.hasOwnProperty('pageCount')) throw new Error('ds.ui.DataController: Result JSON has no property "pageCount" for "dataset" dataType.');
		if (!json.hasOwnProperty('pageIndex')) throw new Error('ds.ui.DataController: Result JSON has no property "pageIndex" for "dataset" dataType.');
		if (!json.hasOwnProperty('pageRowCount')) throw new Error('ds.ui.DataController: Result JSON has no property "pageRowCount" for "dataset" dataType.');
		if (!json.hasOwnProperty('rowCount')) throw new Error('ds.ui.DataController: Result JSON has no property "rowCount" for "dataset" dataType.');
		if (!json.hasOwnProperty('data')) throw new Error('ds.ui.DataController: Result JSON has no property "data" for "dataset" dataType.');
		if (json.data.length != json.pageRowCount) throw new Error('ds.ui.DataController: "pageRowCount" and "data.length" not equal.')
		return json;
	},
	async _getData(page_index) {
		const self = this;
		if (!self.ready) throw new Error('ds.ui.DataController: Is not ready yet.');
		let page = self.data.pages[page_index];
		if (!page.loaded) {
			page.data = (await self._fetch(page.url)).data;
			page.loaded = true;
			self.done = self.data.pages.every(p => p.loaded);
		}
		return page.data;
	},
	async load() {
		const self = this;
		if (!self.url) throw new Error('ds.ui.DataController: URL is required.');
		if (self.ready) await self._dealloc();
		self.ready = false;
		let page0 = await self._fetch(self.url);
		self.data = {
			dealloc: page0.dealloc,
			pageCount: page0.pageCount,
			rowCount: page0.rowCount,
			pages: ds.times(page0.pageCount).map(i => ({ loaded: false, index: i, url: page0.pageList[i], data: null }))
		};
		self.data.pages[0] = {
			loaded: true,
			index: 0,
			url: page0.pageList[0],
			data: page0.data
		}
		self.ready = true;
		self._trigger('ready');
	...
	getCursor() {
		const self = this;
		return ds.ui.DataController.new({ dataController: self });
	}
}, ds.Events('error', 'ready', 'dealloc'));
ds.ui.DataControllerCursor = ds.Object.extend({
	dataController: null,
	pageIndex: -1,
	done: false,
	async fetch() {
		const self = this;
		if (self.done) return [];
		let data = await self.dataController._getData(++self.pageIndex);
		let done = self.pageIndex == (self.dataController.pageCount - 1);
		return { data, done };
	},
	async fetchRest() {
		const self = this;
		throw new Error('fetchRest not implemented yet.');
	},
	async fetchAll() {
		const self = this;
		let data = [];
		for (let i = 0; i < self.dataController.pageCount; i++) data = [...data, ...(await self.dataController._getData(i))];
		self.pageIndex = self.dataController.pageCount -1;
		self.done = true;
		return { data, done: true };
	},
	init() {
		const self = this;
		if (!self.dataController) throw new Error('ds.ui.DataControllerCursor: dataController is required.');
	}
});

ds.ui.VirtualList = ds.ui.View.extend({
	template: `<div class="scroll bkw bl br bt bb">
					<div ref="body_element" style="position: relative; height:{{ this._body_height }}px;">
						{{ this._getBody() }}
					</div>
				</div>`,
	_body_height: 0,
	...
	_getBody() {
		const self = this;
		return (self._rows || []).filter(row => row.visible).map(row => row.element);
	},
	update() {
		const self = this;
		const width = self.element.clientWidth;
		const height = self.element.clientHeight;
		self._body_height = 0;
		let top = 0;
		(self._rows || []).forEach(row => {
			if (!row.element) {
				row.element = ds.ui.element(`<div style="position: absolute; width:${width}px;"></div>`);
				row.cell = ds.ui.Cell.new({ text: row.item.name, image: '/assets/images/docin.svg', className: 'ml mt mr mb' });
				row.element.appendChild(row.cell.element);
			}
			row.top = top;
			row.height = row.element.clientHeight;

			row.element.style.setProperty('top', `${row.top}px`);

			let scroll_top = row.top - self.element.scrollTop;
			let scroll_bottom = scroll_top + row.height;

			row.visible = (scroll_top >= 0 && scroll_top <= height) || (scroll_bottom >= 0 && scroll_bottom <= height);

			self._body_height += row.height;
			top += row.height;
		});
		ds.ui.View.update.call(self);
	},
	setData(data) {
		const self = this;
		self._rows = data.map(item => ({ item: item, element: null, cell: null, top: null, height: null, visible: false }));
		self.needsUpdate();
	},
	init() {
		const self = this;
	...
	/// test...
	async test() {
		let vl = ds.ui.VirtualList.new({ className: 'abs', style: { left: '10px', top: '10px', width: '300px', height: '500px' } });
		document.body.appendChild(vl.element);
		let data = await sbxapp.SBXApp.shared().connection.request('get', '/find?query=object');
		vl.setData(data);
	}
})
...
// 3.1 paste yaml and save as .sublime-syntax
// 4. patch polymer with:
/*		- match: '{{'
...
// 5. open Monokai color and paste block for double curly braces
/*
%YAML 1.2
---
